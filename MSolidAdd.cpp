...
bool MSolidAdd::CheckCollisions(bool falling_check)
{
    ...
    Obj_Type_t BlkLeftTop = ModMapObj->gmap[map_cols_pos.x_left][map_cols_pos.y_top].type;
    Obj_Type_t BlkRightTop = ModMapObj->gmap[map_cols_pos.x_right][map_cols_pos.y_top].type;
    Obj_Type_t BlkLeftBtm = ModMapObj->gmap[map_cols_pos.x_left][map_cols_pos.y_bottom].type;
    Obj_Type_t BlkRightBtm = ModMapObj->gmap[map_cols_pos.x_right][map_cols_pos.y_bottom].type;

    MSG_DBG(4, "map_cols_pos.x_left:%d map_cols_pos.y_bottom:%d map_cols_pos.y_under_bottom:%d", map_cols_pos.x_left,
	    map_cols_pos.y_bottom, map_cols_pos.y_under_bottom);
    MSG_DBG(4, "BlkLeftTop:%d  BlkRightTop:%d  BlkLeftBtm:%d  BlkRightBtm:%d", BlkLeftTop, BlkRightTop, BlkLeftBtm,
	    BlkRightBtm);

    if (falling_check) { // проверка падение
	Obj_Type_t BlkLeftUBtm = ModMapObj->gmap[map_cols_pos.x_left][map_cols_pos.y_under_bottom].type; // next pixel
	Obj_Type_t BlkRightUBtm = ModMapObj->gmap[map_cols_pos.x_right][map_cols_pos.y_under_bottom].type; //
	MSG_DBG(4, "BlkLeftUBtm:%d  BlkRightUBtm:%d", BlkLeftUBtm, BlkRightUBtm);

	//ниже спрайта не свободно и позиция на границе ячейки
	if ((BLK_NOT_FREE(BlkLeftUBtm) || BLK_NOT_FREE(BlkRightUBtm))
		&& cols_pos.y_bottom == Y_TO_DISP_COORD_EXT(map_cols_pos.y_under_bottom) - 1)
	{ // приземлились
	    if (jump_mode > 0) {
...
//				Move(0, false, false);
		...
		MSG_DBG(4, "****03****");
		dy = 1;
		GObj.SetActiveSpriteBeginIndex(0); //TODO
	    }
	}
    }
...
//		MSG_DBG(2, "****** _speed:%d",speed);
    ...
    if (jump_mode > 0) {
	if (last_to_right)
	    sprite_set_curr = EObject::Sprite_Jump_Right_Sheet;
	else
	    sprite_set_curr = EObject::Sprite_Jump_Left_Sheet;
    } else {
	jump_speed = DEF_JUMP_SPEED;
	if (!dx)
	    sprite_show_mode = 0;
	...
	if (last_to_right)
	    ...
	    sprite_set_curr = EObject::Sprite_Left_Sheet;
    }
...
//		MSG_DBG(2, "+++++++ Jump :%d  x_drw:%d, y_drw:%d", jump_mode, x_drw, y_drw);
	...
	jump_speed += time_flag;

//	MSG_DBG(2, "****** jump_speed:%d", jump_speed);
    }
